...
export var AbstractEditor = Class.extend({
  ...
  init: function (options, defaults) {
    ...
    this.key =
      this.parent !== undefined ? this.path.split('.').slice(this.parent.path.split('.').length).join('.') : this.path;
  ...
  expandCallbacks: function (scope, options) {
    for (var i in options) {
      if (options.hasOwnProperty(i) && options[i] === Object(options[i])) {
        ...
        typeof this.defaults.callbacks[scope] === 'object' &&
        typeof this.defaults.callbacks[scope][options[i]] === 'function'
      ) {
