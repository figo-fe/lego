...
export var MultiSelectEditor = AbstractEditor.extend({
  ...
  build: function () {
    ...
    this.multiselectChangeHandler = function (e) {
      ...
      for (i = 0; i < self.option_keys.length; i++) {
        if (
          self.select_options[self.option_keys[i]] &&
          (self.select_options[self.option_keys[i]].selected || self.select_options[self.option_keys[i]].checked)
        )
          newValue.push(self.select_values[self.option_keys[i]]);
      }
  ...
  removeValue: function (value) {
    ...
    value = [].concat(value);
    this.setValue(
      this.getValue().filter(function (item) {
