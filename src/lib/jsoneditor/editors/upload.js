    ...
    if (!this.schema.readOnly && !this.schema.readonly) {
      ...
      this.uploadHandler = function (e) {
        ...
        if (files && files.length) {
          if (self.options.max_upload_size !== 0 && files[0].size > self.options.max_upload_size) {
            ...
            self.options.mime_type.length !== 0 &&
            !self.isValidMimeType(files[0].type, self.options.mime_type)
          ) {
            self.theme.addInputError(
              self.uploader,
              'Wrong file format. Allowed format(s): ' + self.options.mime_type.toString(),
            );
          } else {
      ...
      this.dragHandler = function (e) {
        ...
        var validType =
          files &&
          files.length &&
          (self.options.mime_type.length === 0 || self.isValidMimeType(files[0].type, self.options.mime_type));
